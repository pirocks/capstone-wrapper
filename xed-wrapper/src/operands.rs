...
use wrapper_common::memory_operand::{GeneralRegister, X86Scale};
...
impl MemoryOperands {
    pub fn from_xed(instr: *const xed_decoded_inst_t, _instr_template: *const xed_inst_t, mem_idx: c_uint) -> Self {
        unsafe {
            ...
            let index = GeneralRegister::try_new(index_raw).unwrap();
            let base = GeneralRegister::try_new(base_reg_raw).unwrap();
            Self::SIBAddressing {
