...
pub fn adc_generic<T1, T2, D1: Writeable<T1>, S1: Readable<T1>, S2: Readable<T2>>(
        ...
        match adc {
            ...
            ADC::ADC_GPRV_IMMZ {
                ...
                GeneralReg::Reg64(reg) => {
                    adc_generic(
                        semantics,
                        semantics.get_reg_64(reg),
                        semantics.get_reg_64(reg),
                        operand_1.unwrap_u64(),
                    );
                }
                GeneralReg::Reg32(reg) => {
                    adc_generic(
                        semantics,
                        semantics.get_reg_32(reg),
                        semantics.get_reg_32(reg),
                        operand_1.unwrap_i32(),
                    );
                }
                GeneralReg::Reg16(reg) => {
                    adc_generic(
                        semantics,
                        semantics.get_reg_16(reg),
                        semantics.get_reg_16(reg),
                        operand_1.unwrap_i16(),
                    );
                }
                GeneralReg::Reg8(reg) => {
                    adc_generic(
                        semantics,
                        semantics.get_reg_8(reg),
                        semantics.get_reg_8(reg),
                        operand_1.unwrap_i8(),
                    );
                }
