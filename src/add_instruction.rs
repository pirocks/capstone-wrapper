...
impl AddInstruction {
    ...
    fn with_target8(next_operand: &X86Operand, target: MemoryOperandOrRegister8) -> AddInstruction {
        match &next_operand.op_type {
            X86OperandType::Reg(reg) => {
                AddInstruction::Rm8R8 { target, to_add: Register8::new(*reg) }
            }
            X86OperandType::Imm(imm) => {
                assert_eq!(OperandSize::from_capstone_size(next_operand.size), OperandSize::HalfWord);
                AddInstruction::Rm8imm8 { target, imm8: imm_i8(*imm) }
            }
            X86OperandType::Mem(mem) => {
                AddInstruction::R8Rm8 { target: target.unwrap_reg(), to_add: MemoryOperandOrRegister8::Mem(MemoryOperand::from_mem(mem)) }
            }
            X86OperandType::Invalid => {
                todo!()
            }
    ...
    fn with_target16(next_operand: &X86Operand, target: MemoryOperandOrRegister16) -> AddInstruction {
        match &next_operand.op_type {
            X86OperandType::Reg(reg) => {
                AddInstruction::Rm16R16 { target, to_add: Register16::new(*reg) }
            }
            X86OperandType::Imm(imm) => {
                match OperandSize::from_capstone_size(next_operand.size) {
                    OperandSize::QuadWord |
                    OperandSize::DoubleWord => {
                        panic!("Unexpected large constant")
                    }
                    OperandSize::Word => {
                        AddInstruction::Rm16imm16 { target, imm16: imm_i16(*imm) }
                    }
                    OperandSize::HalfWord => {
                        AddInstruction::Rm16imm8 { target, imm8: imm_i8(*imm) }
                    }
                }
            }
            X86OperandType::Mem(mem) => {
                AddInstruction::R16Rm16 { target: target.unwrap_reg(), to_add: MemoryOperandOrRegister16::Mem(MemoryOperand::from_mem(mem)) }
            }
            X86OperandType::Invalid => {
                todo!()
            }
    ...
    fn with_target32(next_operand: &X86Operand, target: MemoryOperandOrRegister32) -> AddInstruction {
        match &next_operand.op_type {
            X86OperandType::Reg(reg) => {
                AddInstruction::Rm32R32 { target, to_add: Register32::new(*reg) }
            }
            X86OperandType::Imm(imm) => {
                match OperandSize::from_capstone_size(next_operand.size) {
                    OperandSize::DoubleWord => {
                        AddInstruction::Rm32imm32 { target, imm32: imm_i32(*imm) }
                    }
                    OperandSize::QuadWord |
                    OperandSize::Word => {
                    ...
                    OperandSize::HalfWord => {
                        AddInstruction::Rm32imm8 { target, imm8: imm_i8(*imm) }
                    }
                }
            }
            X86OperandType::Mem(mem) => {
                AddInstruction::R32Rm32 { target: target.unwrap_reg(), to_add: MemoryOperandOrRegister32::Mem(MemoryOperand::from_mem(mem)) }
            }
            X86OperandType::Invalid => {
                todo!()
            }
    ...
    fn with_target64(next_operand: &X86Operand, target: MemoryOperandOrRegister64) -> AddInstruction {
        match &next_operand.op_type {
            X86OperandType::Reg(reg_id) => {
            ...
            X86OperandType::Imm(imm) => {
                match OperandSize::from_capstone_size(next_operand.size) {
                    OperandSize::QuadWord => {
                        AddInstruction::Rm64imm32 { target, imm32: imm_i32(*imm) }
                    }
                    OperandSize::DoubleWord |
                    OperandSize::Word => {
                        panic!("Unexpected constant size")
                    ...
                    OperandSize::HalfWord => {
                        AddInstruction::Rm64imm8 { target, imm8: imm_i8(*imm) }
                    }
                }
            }
            X86OperandType::Mem(mem) => {
                AddInstruction::R64Rm64 { target: target.unwrap_reg(), to_add: MemoryOperandOrRegister64::Mem(MemoryOperand::from_mem(mem)) }
            }
            X86OperandType::Invalid => {
                todo!()
            }
...
pub mod test {
    ...
    pub fn disassemble_add_instruction_variants() {
        ...
        assert_eq!(&res[1], &X86Instruction::Add(AddInstruction::Rm8imm8 {
            target: MemoryOperandOrRegister8::Reg(Register8::AL),
            imm8: -9,
        }));
        assert_eq!(&res[2], &X86Instruction::Add(AddInstruction::Rm16imm16 {
            target: MemoryOperandOrRegister16::Reg(Register16::AX),
            imm16: 3,
        }));
        assert_eq!(&res[3], &X86Instruction::Add(AddInstruction::Rm32imm32 {
            target: MemoryOperandOrRegister32::Reg(Register32::EAX),
            imm32: -9,
        }));
        assert_eq!(&res[4], &X86Instruction::Add(AddInstruction::Rm64imm32 {
            target: MemoryOperandOrRegister64::Reg(Register64::RAX),
            imm32: -9,
        }));
        assert_eq!(&res[5], &X86Instruction::Add(AddInstruction::Rm8imm8 {
            target: MemoryOperandOrRegister8::Mem(MemoryOperand::base64(Register64::RCX)),
            imm8: -9,
        }));
        assert_eq!(&res[6], &X86Instruction::Add(AddInstruction::Rm16imm16 {
            target: MemoryOperandOrRegister16::Mem(MemoryOperand::base64(Register64::RDX)),
            imm16: -99,
        }));
        assert_eq!(&res[7], &X86Instruction::Add(AddInstruction::Rm32imm32 {
