...
pub(crate) fn has_a_label_expr(expr: &KExpression, label: &str) -> bool {
    match expr {
        ...
        KExpression::KRewrite { lhs, rhs } => {
            has_a_label_expr(lhs, label) || has_a_label_expr(rhs, label)
        }
...
pub fn has_execinstr_label(sentence: &KSentence, label: &str) -> bool {
    match sentence {
        ...
        KSentence::KRule {
            ...
            has_a_label_expr(body, label)
                || has_a_label_expr(requires, label)
                || has_a_label_expr(ensures, label)
        }
...
pub fn extract_apply_label(expr: &KExpression) -> &str {
    match expr {
        KExpression::KApply { label, .. } => label.as_str(),
        _ => panic!(),
    }
}
