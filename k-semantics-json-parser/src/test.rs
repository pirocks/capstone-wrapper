...
pub fn test_extract_vfmadd213sd_xmm_xmm_xmm() -> anyhow::Result<()> {
    let top_level: TopLevel = serde_json::from_reader(BufReader::new(File::open("data/minimized-VFMADD213SD-XMM-XMM-XMM.json")?))?;
    let _rule = extract_rule_from_semantics(top_level, &InstructionDescriptor {
        operands: vec![OperandType::Reg(RegisterType::AllXmm32), OperandType::Reg(RegisterType::AllXmm32), OperandType::Reg(RegisterType::AllXmm32)],
...
pub fn test_extract_movq_r64_r64() -> anyhow::Result<()> {
    let top_level: TopLevel = serde_json::from_reader(BufReader::new(File::open("data/minimized-MOVQ-R64-R64.json")?))?;
    let _rule = extract_rule_from_semantics(top_level, &InstructionDescriptor {
